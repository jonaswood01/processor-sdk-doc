...
`3.1.1.1.4. Boot Flow <https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/Foundational_Components/U-Boot/UG-General-Info.html#boot-flow>`_
for more info about the AM62x boot flow. OpenOCD/GDB and CCS are useful tools to debug source
...
Use OpenOCD/GDB and CCS on a Linux host machine to debug SPL on the SK-AM62B-P1 board,
either in the *board_init_f()* function or upon R5 or A53 processor reset.

.. note::

   First, ensure you're able to:

      - Clone, access, and modify the *ti-u-boot* source code: `3.1.1.1.1. Getting the U-Boot Source Code <https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/Foundational_Components/U-Boot/UG-General-Info.html#getting-the-u-boot-source-code>`_
      - Build u-boot: `3.1.1.1.2. Build U-Boot <https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/Foundational_Components/U-Boot/UG-General-Info.html#build-u-boot>`_ or `1.1.4. SDK Build using Makefile <https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/Overview/Top_Level_Makefile.html#sdk-build-using-makefile>`_
      - Create an SD card: `1.1.2. Create SD Card <https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/Overview/Processor_SDK_Linux_create_SD_card.html#create-sd-card>`_
      - Boot up and interface with the SK-AM62B-P1 board: `4.2.1. AM62x SK EVM Hardware Setup <https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/How_to_Guides/Hardware_Setup_with_CCS/AM62x_EVM_Hardware_Setup.html#am62x-sk-evm-hardware-setup>`_

   before continuing with the below debugging example.

#. Add a debug dead loop:

   **Option A**: in the *board_init_f()* function in the *am625_init.c* source code file:

      #. Open the *am625_init.c* source code at the file path:

         .. code-block:: console

            ti-u-boot/arch/arm/mach-k3/am62x/am625_init.c
   ...
   **Option B**: at R5 or A53 processor *reset* in the *start.S* source code file:

      #. Open the *start.S* source code at the file path:

         **R5:**

         .. code-block:: console

            ti-u-boot/arch/arm/cpu/armv7/start.S

         **A53:**

         .. code-block:: console

            ti-u-boot/arch/arm/cpu/armv8/start.S
      ...
      #. Add a debug dead loop at *reset*:
         ...
         **A53:**
         ...
         - These dead loops are in assembly and are specific to the R5 or A53 processor in
           the AM62x, based on which *start.S* file you opened and modified above.

   .. note::

      On AM335x/AM437x/AM57xx, a debug dead loop could be added in the *early_system_init* function at
      the file path *ti-u-boot/arch/arm/mach-omap2/hwinit-common.c*. The location of a debug dead loop
...
#. Move the modified u-boot binaries to the SD card:
   ...
   After building u-boot, copy/replace the *tiboot3.bin*, *tispl.bin*, and *u-boot.img*
   u-boot binaries to the SD card's boot partition and insert the SD card in the board.

   .. note::
      On AM335x/AM437x/AM57xx, copy *MLO* and *u-boot.img* to the SD card. On AM65x, copy
      *sysfw.itb*, *tiboot3.bin*, *tispl.bin* and *u-boot.img* to the SD card.
...
#. Download, install, and setup OpenOCD and GDB-Multiarch: https://docs.u-boot.org/en/latest/board/ti/k3.html#step-1-download-and-install-openocd
...
#. Leave the OpenOCD terminal running and open a separate terminal to connect GDB and debug:
   ...
   **Option A**: on R5 (for this example) in the *board_init_f()* function in the *am625_init.c* source code file:
   ...
   - Add the symbol file that came from building u-boot and connect to the R5 core with GDB:
      ...
      .. code-block:: console
         ...
         board_init_f (dummy=<optimized out>) at /home/${USER}/ti-u-boot/arch/arm/mach-k3/am62x/am625_init.c:194
         194		asm("test: nop");
         (gdb)
      ...
      .. note::
         ...
         On other devices, the *u-boot-spl* symbol file may be located in a different directory upon building u-boot.
   ...
   - Use GDB debugging commands to debug SPL:
      ...
      .. code-block:: console
         ...
         pc             0x43c0099a             0x43c0099a <board_init_f+274>
         (gdb) set $pc = 0x43c0099c
         (gdb) continue
         Continuing.
         <Ctrl + C>

         Program received signal SIGINT, Interrupt.
         board_init_f (dummy=<optimized out>) at /home/${USER}/ti-u-boot/arch/arm/mach-k3/am62x/am625_init.c:202
         (gdb) b spl_mmc_boot_mode
         Breakpoint 1 at 0x43c00bb0: file /home/${USER}/ti-u-boot/arch/arm/mach-k3/am62x/am625_init.c, line 315.
         (gdb) set x = 0
         (gdb) continue
         Continuing.

         Breakpoint 1, spl_mmc_boot_mode (mmc=0x43c34788, boot_device=8) at /home/${USER}/ti-u-boot/arch/arm/mach-k3/am62x/am625_init.c:315
         (gdb) next
         (gdb) step
         (gdb) tui disable
         (gdb) q
   ...
   **Option B**: at A53 processor reset (for this example) in the start.S source code file:
   ...
   - Add the symbol file that came from building u-boot and then connect to the A53 core with GDB:
      ...
      .. code-block:: console
         ...
         warning: No executable has been specified and target does not support
         determining executable automatically.  Try using the "file" command.
         reset () at /home/${USER}/ti-u-boot/arch/arm/cpu/armv8/start.S:58
         58		nop
         (gdb)
   ...
   - Use GDB debugging commands to debug SPL:
      ...
      .. code-block:: console
         ...
         Breakpoint 1 at 0x800800e8: file /home/${USER}/ti-u-boot/arch/arm/cpu/armv8/start.S, line 399.
         (gdb) continue
         Continuing.

         Breakpoint 1, save_boot_params () at /home/${USER}/ti-u-boot/arch/arm/cpu/armv8/start.S:399
         (gdb) next
         save_boot_params_ret () at /home/${USER}/ti-u-boot/arch/arm/cpu/armv8/start.S:125
         (gdb) next
         (gdb) nexti
         (gdb) tui disable
         (gdb) q
...
Refer to the U-Boot K3 OpenOCD/GDB documentation for more info: https://docs.u-boot.org/en/latest/board/ti/k3.html#common-debugging-environment-openocd.
...
#. Download, install, setup, and open CCS (Linux): https://www.ti.com/tool/download/CCSTUDIO.

   .. note::

      This guide uses CCS 12.x. Remember, we will be building U-Boot on a Linux host machine, so it is also
      recommended to install CCS on Linux. Refer to the Linux CCS host support if needed:
...
#. Load symbol files:
   ...
   #. After successful launch, connect to one of the ARM cores:
      ...
      .. note::
         ...
         On AM62x, the inital SPL code is executed on the R5, so the user would connect
         to the R5 core. On AM335x/AM437/AM57xx, connect to the A8/A9/A15 core instead.
   ...
   #. After connecting to the core:
      ...
      - Click "u-boot-spl" with no file extension (you may have to change the file type to "All Files"):
...
.. note::
   ...
   On K3 family of devices such as AM62x, a watchdog timer part of the DMSC is enabled by default by the ROM bootcode
   with a timeout of 3 minutes. The watchdog timer is serviced by System Firmware (SYSFW) during normal operation. If
   debugging SPL before SYSFW is loaded, the watchdog timer will not get serviced automatically and the debug session
   will reset after 3 minutes. Therefore, it is recommended to start debugging SPL code only after the startup of SYSFW
   to avoid running into the watchdog timer reset. Similarly, the watchdog timer is also enabled on the
   AM335x/AM437x/AM57xx devices and please refer to the "Disable_Watchdog()" function in our GEL files.